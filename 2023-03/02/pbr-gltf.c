...
int32_t WINAPI WinMain(HINSTANCE instance, HINSTANCE prevInstance, LPSTR cmdLine, int32_t showWindow) {
    ...
    Program_setMat4Uniform(&program, "proj", (const GLfloat *) &projMatrix);
    Program_setVec3Uniform(&program, "lightPositions[0]", (GLfloat[]) { 5.0f,  10.0f, 5.0f });
    Program_setVec3Uniform(&program, "lightPositions[1]", (GLfloat[]) { -5.0f,  10.0f, 5.0f });
    Program_setVec3Uniform(&program, "lightPositions[2]", (GLfloat[]) { 5.0f, 10.0f, -5.0f });
    Program_setVec3Uniform(&program, "lightPositions[3]", (GLfloat[]) { -5.0f, 10.0f, -5.0f });
    Program_setIntUniform(&program, "colorMap", 0);
    Program_setIntUniform(&program, "normalMap", 1);
    Program_setIntUniform(&program, "metallicRoughnessMap", 2);
    ...
    while (running) {
        ...
        camera_buildMatrix(&camera);
        Program_setMat4Uniform(&program, "view", (const GLfloat *) &camera.matrix);
        Program_setVec3Uniform(&program, "eyePosition",(const GLfloat *) &camera.position);
        ...
        for (int32_t i = 0; i < Vector_Object_size(&objects); ++i) {
            ...
            Program_setMat4Uniform(&program, "world", (const GLfloat *) &object.transform);
