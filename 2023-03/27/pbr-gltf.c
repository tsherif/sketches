...
int32_t WINAPI WinMain(HINSTANCE instance, HINSTANCE prevInstance, LPSTR cmdLine, int32_t showWindow) {
    ...
    size_t programHandle = Database_addProgram(&db, program);
    ...
    for (int32_t i = 0; i < gltf_data->nodes_count; ++i) {
        Object object = { 0 };
        VertexArray vertexArray = VertexArray_create(&db);
        cgltf_node* node = gltf_data->nodes + i;
        parseGLTF(node->mesh, gltf_data->images, &bufferData, &object.mesh);
        initVertexArray(&object.mesh, &vertexArray);

        size_t vertexArrayHandle = Database_addVertexArray(&db, vertexArray);
    ...
    while (running) {
        ...
        for (int32_t i = 0; i < Vector_Object_size(&objects); ++i) {
            ...
            Program programObj = Database_getProgram(&db, object.pipeline.program);
            Program_setVecUniform(&programObj, "view", &camera.matrix);
            Program_setVecUniform(&programObj, "eyePosition", &camera.position);
            Program_setVecUniform(&programObj, "world", &object.transform);
