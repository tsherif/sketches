...
int32_t WINAPI WinMain(HINSTANCE instance, HINSTANCE prevInstance, LPSTR cmdLine, int32_t showWindow) {
    ...
    Program_setVecUniform(&program, "proj", (const GLfloat *) &projMatrix);
    ...
    while (running) {
        ...
        camera_buildMatrix(&camera);
        Program_setVecUniform(&program, "view", (const GLfloat *) &camera.matrix);
        Program_setVecUniform(&program, "eyePosition",(const GLfloat *) &camera.position);
        ...
        for (int32_t i = 0; i < Vector_Object_size(&objects); ++i) {
            ...
            Program_setVecUniform(&program, "world", (const GLfloat *) &object.transform);
