...
int CALLBACK WinMain(HINSTANCE instance, HINSTANCE, LPSTR, int showWindow) {
    ...
    layout(std430, binding=0) buffer Position {
        restrict readonly vec4 position[];
    };
    layout(std430, binding=1) buffer OutPosition {
        restrict writeonly vec4 outPosition[];
    };
    layout(std430, binding=2) buffer Velocity {
        restrict vec4 velocity[];
    };
    layout(std430, binding=3) buffer Mass {
        restrict readonly float mass[];
    };
