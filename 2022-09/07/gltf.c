...
int32_t WINAPI WinMain(HINSTANCE instance, HINSTANCE prevInstance, LPSTR cmdLine, int32_t showWindow) {
    ...
    initTexture(filePath, &tripod.material.colorTexture);
    ...
    snprintf(filePath, 1024, "%s/%s", MODEL_DIR, gltf_data->images[1].uri);    
    initTexture(filePath, &camera.material.colorTexture);
