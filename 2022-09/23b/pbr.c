...
int32_t WINAPI WinMain(HINSTANCE instance, HINSTANCE prevInstance, LPSTR cmdLine, int32_t showWindow) {
    ...
    GLuint metalnessLocation = glGetUniformLocation(program, "metalness");
    GLuint roughnessLocation = glGetUniformLocation(program, "roughness");
    ...
    while (running) {
        ...
        for (int32_t row = 0; row < numRows; ++row) {
            ...
            glUniform1f(metalnessLocation, (float) row / (numRows - 1));

            for (int32_t col = 0; col < numCols; ++col) {
                float x = (col - numCols / 2) * 2.5f;

                glUniform2f(offsetLocation, x, y);
                float roughness =  (float) col / (numCols - 1);
