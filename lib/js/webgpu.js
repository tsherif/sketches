...
export function createRenderPipeline(device, props) {
    ...
    const buffers = vertexBufferCfg.map((format, i) => ({
...
export function createBindings(device, pipeline, props) {
    const { vertexBuffers, uniforms } = props;
    return {
        vertexBuffers,
        bindGroups: uniforms.map((entries, i) => device.createBindGroup({
...
export function createRenderPassConfig(props) {
    const { context, bindings, pipeline, vertices, instances = 1 } = props;
    return {
        context,
        pipeline,
        bindings,
...
export function encodeRenderPass(commandEncoder, config) {
    ...
    const { vertexBuffers, bindGroups} = bindings;

    descriptor.colorAttachments.forEach(c => {
        if (c.view) {
    ...
    vertexBuffers.forEach((b, i) => renderPass.setVertexBuffer(i, b))
    bindGroups.forEach((bg, i) => renderPass.setBindGroup(i, bg))
